...
export default function ImageDisplay({
  ...
  useEffect(() => {
    ...
    if (
      !isProcessing &&
      (processedImage !== null || visualizationImage !== null)
    ) {
  ...
  useEffect(() => {
    ...
    const imageToUse =
      isProcessing && processedImage === null
        ? previousImagesRef.current.processedImage
        : processedImage
  ...
  useEffect(() => {
    // Use current or previous image based on processing state
    const imageToUse =
      isProcessing && visualizationImage === null
        ? previousImagesRef.current.visualizationImage
        : visualizationImage
  ...
  const createCircleCursor = useCallback((size: number): string => {
  ...
  useEffect(() => {
    if (isWhitelistBrushActive) {
      ...
      const visibleSize = Math.max(
        Math.ceil(whitelistBrushSize * zoomLevel),
        10,
      )
      const cursorUrl = createCircleCursor(visibleSize)
      setBrushCursor(
        `url(${cursorUrl}) ${visibleSize / 2} ${visibleSize / 2}, auto`,
      )
    } else {
  ...
  const handleWhitelistBrushSizeChange = (
  ...
  }, [historyIndex, isDrawing, isProcessing, history, onManualWhitelist])
  ...
  }, [historyIndex, history, isDrawing, isProcessing, onManualWhitelist])
  ...
  }, [isWhitelistBrushActive, isProcessing, handleUndo, handleRedo])
  ...
  return (
    ...
    >
      <div style={zoomControlsStyle}>
        ...
        <div style={zoomDisplayStyle}>{Math.round(zoomLevel * 100)}%</div>
        <button
          type="button"
          style={zoomButtonStyle}
        ...
        <div style={brushControlsStyle}>
          <button
            ...
            title={
              isWhitelistBrushActive
                ? 'Disable whitelist brush'
                : 'Enable whitelist brush'
            }
            ...
            <>
              <div style={sliderContainerStyle}>
                ...
                <div style={sliderLabelStyle}>
                  ...
                  <div>
                    Screen Diameter:{' '}
                    {Math.round(whitelistBrushSize * zoomLevel)} px at{' '}
      ...
      <div style={sideBySideContainerStyle}>
        ...
        <div style={sideContainerStyle}>
          <div style={contentPanelStyle}>
            ...
            <div ref={originalScrollContainerRef} style={canvasContainerStyle}>
        ...
        <div style={sideContainerStyle}>
          <div className="tab-buttons" style={tabContainerStyle}>
            <button
              type="button"
              style={
                rightActiveTab === 'processed'
                  ? activeTabStyle
                  : tabButtonBaseStyle
              }
              onClick={() => setRightActiveTab('processed')}
              disabled={
                !processedImage && !previousImagesRef.current.processedImage
              }
            >
              Processed Image
            </button>
          ...
          <div className="tab-content" style={rightTabContentStyle}>
            <div ref={rightScrollContainerRef} style={canvasContainerStyle}>
              {/* Always render both canvases but show/hide based on active tab */}
              <div
                style={{
                  ...canvasWrapperStyle,
                  display:
                    rightActiveTab === 'processed' ? 'inline-block' : 'none',
                }}
              >
                <canvas ref={processedCanvasRef} style={canvasStyle}></canvas>

                {/* Processing overlay for the processed image */}
                {isProcessing && rightActiveTab === 'processed' && (
                  <div style={processingOverlayStyle}>
                    <div style={processingIndicatorStyle}>Processing...</div>
                  </div>
                )}
              </div>
