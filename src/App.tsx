...
function App() {
  ...
  const [visualizationImage, setVisualizationImage] =
    useState<ImageData | null>(null)
  const [processingStats, setProcessingStats] = useState<
    ProcessingResult['stats'] | null
  >(null)
  const [isProcessing, setIsProcessing] = useState(false)
  const [parameters, setParameters] = useState({ ...defaultParams })
  const [debounceTimeout, setDebounceTimeout] = useState<number | null>(null)
  const [manualWhitelistMask, setManualWhitelistMask] = useState<
    boolean[] | undefined
  >(undefined)
  ...
  }, [originalImageData, processImageWithParams, debounceTimeout])

  const handleParameterChange = (
    name: keyof typeof defaultParams,
    value: number,
  ) => {
